        ...
        if(!empty($data)){
            foreach($data as $item){
                if(!empty($item['PROD_NAME'])){
                    $importModel = new ImportFileModel();
                    $checkDuplicateID=$importModel->CheckDuplicateValueForMedicineData($item);
...
//                            'price'=>'',
                ...
                if(!empty($item['COM_CODE']) && !empty($item['COM_NAME'])){
                    ...
                    if(empty($manufacturerID)){
                        $insertData = [
                            ...
                            'registration_dt'=>$this->registrationDt($item['VALID_UPTO']),
                            'status'=>1,
                            'created_at' =>date('Y-m-d H:i:s'),
                        ];
    ...
    public function registrationDt($dt){
        if($dt == "0000-00-00"){
            return "0000-00-00";
        }else{
//            list($y, $m, $d) = explode('-', $dt);
//            dd(Carbon::createFromFormat('m/d/Y', $dt)->format('Y-m-d'));
            return Carbon::createFromFormat('m/d/Y', $dt)->format('Y-m-d');
        }
